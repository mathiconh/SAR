...
class InscripcionDataService {
  ...
  async createInscripcion({carreraId, claseId, idUsuario, pagarMP, vehiculoSeleccionado, fechaSprint}) {
    ...
    result = await http.post(`/createInscripcion?carreraId=${carreraId}&claseId=${claseId}&idUsuario=${idUsuario}&pagarMP=${pagarMP}&vehiculoId=${vehiculoSeleccionado}&fechaSprint=${fechaSprint}`);
    // Testing purposes
    // result.status = 200;
